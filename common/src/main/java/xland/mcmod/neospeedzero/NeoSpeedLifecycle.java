...
public final class NeoSpeedLifecycle {
    ...
    private static void checkExistingThings(ServerPlayer player) {
        ...
        // Advancement
        PlatformAccess.wrap(player.getAdvancements()).ns0$progress().forEach(((advancementHolder, advancementProgress) -> {
    ...
    public static void register() {
        ...
        PlatformEvents.getInstance().whenServerStarting(server -> NeoSpeedServer.getRecordManager(server).loadFromServer());
        PlatformEvents.getInstance().whenServerStopped(server -> {
            NeoSpeedServer.getRecordManager(server).saveToServer();
